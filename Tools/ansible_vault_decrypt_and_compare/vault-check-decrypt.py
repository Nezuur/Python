...
class VaultManager:
    ...
    def get_project_from_git_dir(self):
        try:
            ...
            raise RuntimeError("Error: Unable to retrieve the Git remote URL")
        except gitlab.exceptions.GitlabGetError:
            raise ValueError("Error: GitLab project not found for the current repository")

    def read_password_file(self):
        if os.path.isfile(VAULT_PASSWORD_FILE):
            with open(VAULT_PASSWORD_FILE, 'r') as file:
    ...
    def decrypt_ansible_vault(self, vault_content):
        ...
        # Skip the first line (header) and join the remaining lines
    ...
    def compare_contents(self, content1, content2, branch_name):
        diff = difflib.unified_diff(content1.splitlines(), content2.splitlines(), fromfile='current branch', tofile=branch_name, lineterm='')
        for line in diff:
            if line.startswith('-'):
    ...
    def run_comparison(self, branch):
        try:
            ...
            with open(self.vault_path, 'r') as current_file:
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Decrypt and compare Ansible Vault files in current and target branches')
...
def main():
    ...
    try:
        ...
        raise Exception("Error: Invalid GitLab token", e)
